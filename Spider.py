...
class Spider(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.imgSize=(40,20)
        self.image=pygame.image.load(os.path.join("images", "sp1.png"))
        self.gif.append(pygame.image.load(os.path.join("images", "sp1.png")))
        self.gif.append(pygame.image.load(os.path.join("images", "sp2.png")))
    ...
    def update(self):
        if(self.isActive):
            ...
            if self.direction == 4:
                self.rect.y = self.rect.y+5
            if self.rect.y >= 520: #spider sınırı
                if self.direction == 1:
                    self.direction = 2
                else:
                    self.direction =3
            if self.rect.y <= 420: #spider sınırı
                if self.direction == 2:
                    self.direction = 1
                else:
                    self.direction = 4
            if self.rect.x<-40 or self.rect.x>600:
                self.deactivate()
            self.time-=1
    def activate(self): #spider aktif
        self.isActive=True
        start_position = random.randint(1,2)
